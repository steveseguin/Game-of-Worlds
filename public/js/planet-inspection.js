...
(() => {
    ...
    panel.hidden = true;
    panel.setAttribute('aria-label', 'Planet inspection');
    panel.innerHTML = '<div class="inspection-head"><h2 id="inspectionTitle"></h2><button type="button" id="exitPlanetInspection">Back to map</button></div><div class="inspection-detail" tabindex="0" role="region" aria-label="Planet survey details"><strong id="inspectionIntel"></strong><p id="inspectionFleet"></p><p id="inspectionPort"></p><p id="inspectionCapacity"></p><p id="inspectionTerraform"></p><div class="inspection-controls"><button type="button" data-orbit="-0.3" aria-label="Rotate view left">Rotate left</button><button type="button" data-orbit="0.3" aria-label="Rotate view right">Rotate right</button><button type="button" data-zoom="0.85" aria-label="Zoom in on planet">Zoom in</button><button type="button" data-zoom="1.18" aria-label="Zoom out from planet">Zoom out</button></div></div>';
    ...
    function describe(data) {
        const known = !data.sensorContactOnly;
        ...
        text('inspectionTitle', `Planet ${Number(data.id)}`);
        text('inspectionIntel', data.intelMemory ? 'Dated probe report — orbital positions are illustrative' : known ? 'Live survey — orbital positions are illustrative' : 'Sensor contact — probe for a full survey');
        text('inspectionFleet', known ? `${count(ships)} ships in orbit · ${defense} orbital defenses` : 'Fleet composition and defenses are not surveyed.');
    ...
    button.addEventListener('click',()=>{
        const data=window.GAME_STATE?.selectedSectorData;
    ...
    setInterval(()=>{
        const data=window.GAME_STATE?.selectedSectorData;
        button.hidden=!(data && Number(data.type)>=6 && Number(data.type)<=10 && window.Galaxy3D?.inspectPlanet);
        if(activeId!==null){
