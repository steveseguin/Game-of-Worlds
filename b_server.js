...
function surroundShips(message, connection){
	...
	client.query('SELECT * FROM map'+connection.gameid,
			...
			connection.sendUTF(sendchunk);
			if (sendchunk==""){
				connection.sendUTF('You have no ships in nearby sectors to send to that sector.');
			}
			else {
...
function preMoveFleet(message,connection){
	...
	msid = parseInt(message.utf8Data.split(":")[1],16);
			...
			for (i in results){
			    ...
			    for (x=2;x<=(arr.length-1);x+=3){
			      if (parseInt(arr[x],16)==mapa['sectorid'] && arr[x+2]!=undefined && arr[x+1]!=undefined){
                                if ((msid-1)%16 > 8 && msid%16 != 0 ){
                                        if (mapa['sectorid']+1 == msid || mapa['sectorid']-1 == msid || mapa['sectorid']+8 == msid || mapa['sectorid']+9 == msid || mapa['sectorid']-8 == msid || mapa['sectorid']-7 == msid){
						if (mapa['totalship1']>=arr[x+2] && arr[x+1]==1){
							s1+=arr[x+2];
							mapa['totalship1']-=arr[x+2];
						}
						else if (mapa['totalship2']>=arr[x+2] && arr[x+1]==2){
                                                        s2+=arr[x+2];
                                                        mapa['totalship2']-=arr[x+2];
                                                }
                                                else if (mapa['totalship3']>=arr[x+2] && arr[x+1]==3){
                                                        s3+=arr[x+2];
                                                        mapa['totalship3']-=arr[x+2];
                                                }
                                                else if (mapa['totalship4']>=arr[x+2] && arr[x+1]==4){
                                                        s4+=arr[x+2];
                                                        mapa['totalship4']-=arr[x+2];
                                                }
                                                else if (mapa['totalship5']>=arr[x+2] && arr[x+1]==5){
                                                        s5+=arr[x+2];
                                                        mapa['totalship5']-=arr[x+2];
                                                }
                                                else if (mapa['totalship6']>=arr[x+2] && arr[x+1]==6){
                                                        s6+=arr[x+2];
                                                        mapa['totalship6']-=arr[x+2];
                                                }
                                        }
                                }
                                else if ((msid-1)%16 < 7 && msid%16 != 1){
		...
		battleFunction(msid,s1,s2,s3,s4,s5,s6);
                }
        );
				...
				if (results['ownerid']==0){
					owner="no one";
				        if (results['sectortype']>5){
                                	        connection.sendUTF("Our probe was unable to detect enemy life in sector "+sect2update.toString(16)+" and that the sector contains a colonizable planet with a metal production rate of "+results['metalbonus']+"% and a crystal output rate of "+results['crystalbonus']+"%.");
	                                }
						...
						connection.sendUTF("Our probe was destroyed when entering sector "+sect2update.toString(16)+".  No information was gathered.");
                	                        ...
                	                        connection.sendUTF("Our probe was unable to detect enemy life in sector "+sect2update.toString(16)+" and that the sector contains no colonizable planets.");
                        	        }
				}
				else {
					...
					client.query('SELECT * FROM players'+connection.gameid,
						 ...
						 if (spylevel >=0){
			                                if (results['sectortype']>5){
                        			                proberesult = "Our probe detected that sector "+sect2update.toString(16)+" is controlled by Player "+owner+" and that the sector contains a colonizable planet with a metal production rate of "+result['metalbonus']+"% and a crystal output rate of "+result['crystalbonus']+"%.";
                                			}
                                			else {
				...
				if (results['sectorid']==fromHere && results['ownerid']==connection.name){
					if (results['totalship1']>=s1 && results['totalship2']>=s2 && results['totalship3']>=s3 && results['totalship4']>=s4 && results['totalship5']>=s5 && results['totalship6']>=s6){
						...
						if (((fromHere%8>=gotoHere%8-1) || (fromHere%8+8==gotoHere%8+1)) && (fromHere%8<=gotoHere%8+1 || fromHere%8+1==gotoHere%8+8 ||  fromHere%8+1==gotoHere%8+9) && (fromHere-gotoHere<=8) && (gotoHere-fromHere<=9)){
							...
							battleFunction(gotoHere,s1,s2,s3,s4,s5,s6);
							break;
                                                }
                                                else {
...
function battleFunction(gotoHere,s1,s2,s3,s4,s5,s6,resultss){
							client.query('SELECT * FROM players'+connection.gameid,
									...
									if ((s1+s2*3+s3*1+s4*2+s5*3+s6*2)*100<=resultp['crystal']){
									  ...
									  for (var ii in resultss){
										...
										if (resultsx['sectortype']==1){
											...
											client.query('UPDATE players'+connection.gameid+' SET crystal = crystal - '+(s1+s2*3+s3*1+s4*2+s5*3+s6*2)*100+' WHERE playerid = '+connection.name);
											connection.sendUTF('Fleet moved... but the sector contained a blackhole! UH-OH! Cost: '+(s1+s2*2+s3*2+s4+s5*2+s6*3)*100+' Crystal, plus our fleet was crushed by the immense gravity of the black hole!');
											...
											client.query('UPDATE players'+connection.gameid+' SET crystal = crystal - '+(s1+s2*3+s3*1+s4*2+s5*3+s6*2)*100+' WHERE playerid = '+connection.name);
											connection.sendUTF('Fleet moved sucessfully. Cost: '+(s1+s2*3+s3*1+s4*2+s5*3+s6*2)*100+' Crystal.');
											updateAllSectors(connection.gameid,connection);
											updateResources(connection);
											break;
                                                        			}
		...
		if (connection.sectorid!==undefined){
		    connection.sendUTF('Building a ship in sector: '+connection.sectorid);
                    client.query('SELECT * FROM map'+connection.gameid+' WHERE sectorid = "'+connection.sectorid+'" LIMIT 1',
                           ...
                           results2=results2[0];
			   if ((results2['shipyardlvl']>=3 && ship2buy==1) || (results2['shipyardlvl']>=5 && ship2buy==2) || (results2['shipyardlvl']>=1 && ship2buy==3) || (results2['shipyardlvl']>=10 && ship2buy==4) || (results2['shipyardlvl']>=15 && ship2buy==5) || (results2['shipyardlvl']>=7 && ship2buy==6) ){
  			     client.query('SELECT * FROM players'+connection.gameid+' WHERE playerid = "'+connection.name+'" LIMIT 1',
                                function (err, results, fields) {
                                	if (err) {
...
function buyTech(message, connection){
		...
		type2buy = parseInt(tech2buy) ;
			...
			results=results[0];
		 		if (results['tech'+type2buy]!=undefined  && Math.pow(10,results['tech'+type2buy]+1) <= results['research']){
					console.log("TECHID: "+ tech2buy + ", has tech level: "+results['tech'+type2buy]);
					console.log('UPDATE players'+connection.gameid+' SET research = "'+ (results['research'] -  Math.pow(10,results['tech'+type2buy]+1)) +'", tech'+type2buy+' = "tech'+type2buy+' + 1" WHERE  playerid = "'+connection.name+'"')
						client.query('UPDATE players'+connection.gameid+' SET research = "'+ (results['research'] -  Math.pow(10,results['tech'+type2buy]+1)) +'", tech'+type2buy+' = tech'+type2buy+' + 1 WHERE  playerid = "'+connection.name+'"');
						updateResources(connection);
