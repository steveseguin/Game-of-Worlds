// Map and Sector handling code for Galaxy Conquest
...
function calculateResourceProduction(sector, techLevels) {
    ...
    // Apply tech bonuses
    const metalWithTech = metalProduction * Math.pow(1.1, techLevels.metal || 0);
    const crystalWithTech = crystalProduction * Math.pow(1.1, techLevels.crystal || 0);
    const researchWithTech = researchProduction * Math.pow(1.1, techLevels.research || 0);
...
/**
 * Format sector information for client
 * @param {object} sector - Sector data from database
 * @return {string} - Formatted sector message for client
 */
function formatSectorMessage(sector) {
    let message = `sector:${sector.sectorid.toString(16).toUpperCase()}:`;
    message += `owner:${sector.ownerid}:`;
    message += `type:${sector.sectortype}:`;
    message += `artifact:${sector.artifact}:`;
    message += `metalbonus:${sector.metalbonus}:`;
    message += `crystalbonus:${sector.crystalbonus}:`;
    message += `terraform:${sector.terraformlvl}`;
    
    return message;
}

/**
 * Format building information for client
 * @param {object} sector - Sector data from database
 * @return {string} - Formatted building message for client
 */
function formatBuildingMessage(sector) {
    let message = `ub:${sector.metallvl}:${sector.crystallvl}:`;
    message += `${sector.academylvl}:${sector.shipyardlvl}:`;
    message += `${sector.orbitalturret}:${sector.warpgate}`;
    
    return message;
}

/**
 * Format fleet information for client
 * @param {object} sector - Sector data from database
 * @return {string} - Formatted fleet message for client
 */
function formatFleetMessage(sector) {
    let message = `fleet:${sector.totalship1}:${sector.totalship2}:`;
    message += `${sector.totalship3}:${sector.totalship4}:`;
    message += `${sector.totalship5}:${sector.totalship6}:`;
    message += `${sector.totalship7}:${sector.totalship8}:`;
    message += `${sector.totalship9}:`;
    
    // Add ships under construction
    message += `${sector.totship1build}:${sector.totship2build}:`;
    message += `${sector.totship3build}:${sector.totship4build}:`;
    message += `${sector.totship5build}:${sector.totship6build}:`;
    message += `${sector.totship7build}:${sector.totship8build}:`;
    message += `${sector.totship9build}`;
