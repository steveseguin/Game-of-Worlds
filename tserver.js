			   ...
			   if (mapa['ownerid']==connection.name && (mapa['totalship1'] || mapa['totalship2'] || mapa['totalship3'] || mapa['totalship4'] || mapa['totalship5'] || mapa['totalship6'])){
				...
				if ((msid-1)%16 > 8 && msid%16 != 0 ){
				        if (mapa['sectorid']+1 == msid || mapa['sectorid']-1 == msid || mapa['sectorid']+8 == msid || mapa['sectorid']+9 == msid || mapa['sectorid']-8 == msid || mapa['sectorid']-7 == msid){
                                	        sendchunk+=":"+mapa['sectorid'].toString(16).toUpperCase()+":"+mapa['totalship1']+":"+mapa['totalship2']+":"+mapa['totalship3']+":"+mapa['totalship4']+":"+mapa['totalship5']+":"+mapa['totalship6'];
                                	}
				}
	  ...
	  connection.sendUTF('You do not have enough crystal to send this fleet. Needed:'+(sumofships*100));
           ...
           client.query('SELECT * FROM map'+connection.gameid,
			...
			for (i in results){
				...
				console.log("Sector supposedly contains ship found");
				 if (((msid-1)%16 > 8 && msid%16 != 0 ) || resultsx['warpgate']==1){
					console.log("YES THIS SHOUDL WORK kinda!!");
			...
			if (totcrystalcost>0){
				...
				connection.sendUTF("Our fleet successfully departed at a cost of "+totcrystalcost+" crystal and should arrive in sector "+gotoHere.toString(16).toUpperCase()+" in 15 seconds.");
			...
			console.log('end travel function ready');
        		resultp=resultsp[0];			
					...
					if (resultsx['sectortype']==1){
						...
						connection.sendUTF("info:"+gotoHere.toString(16).toUpperCase()+":0");
                                        }
					if (endtravel==0){
					}
...
while (stopvar == 0){
	...
	if ((a1.length + a2.length + a3.length + a4.length + a5.length + a6.length)==0) {   /// if true, all attacking ships are dead. Defender auto-win.
		...
		client.query('UPDATE map'+gID+' SET totalship1 = '+a1.length+', totalship2 = '+a2.length+', totalship3 = '+a3.length+', totalship4 = '+a4.length+', totalship5 = '+a5.length+', totalship6 = '+a6.length+', ownerid = '+pID+', colonized = 0, groundturret = 0, orbitalturret = 0, warpgate = 0, academylvl = 0, shipyardlvl = 0, metallvl = 0, crystallvl = 0, totship1build = 0, totship2build = 0, totship3build = 0, totship4build = 0, totship5build = 0, totship6build = 0 WHERE sectorid = '+gotoHere);
...
function updateSectorProbe(sect2update, connection){
				...
				if (results['ownerid']==0){
					...
					client.query('SELECT * FROM players'+connection.gameid,
						 ...
						 if (spylevel == 1){proberesult += "Our spies were able to detect that the enemy has at least "+Math.floor(Math.random()*totships)+" ships in sector "+sect2update.toString(16).toUpperCase()+".";}
						 if (spylevel == 2){proberesult += "Our spies were able to detect that the enemy has "+totships+" ships in sector "+sect2update.toString(16).toUpperCase()+".";}
						 if (spylevel == 3){proberesult += "Our spies were able to detect that the enemy has "+totships+" ships in sector "+sect2update.toString(16).toUpperCase()+" and "+probee['orbitalturret']+" orbital turrets.";}
 						 if (spylevel >= 4){proberesult += "Our spies were able to detect that the enemy has "+totships+" ships in sector "+sect2update.toString(16).toUpperCase()+", "+probee['orbitalturret']+" orbital turrets, and "+probee['warpgate']+ " warp gate(s)." ;}
						 if (spylevel == -1){proberesult = "Our probe was destroyed by Player "+owner+" when attempting to enter sector "+sect2update.toString(16).toUpperCase()+".  No other information was gathered.";
                                                        for (cc in clients){
						 ...
						 if (spylevel <= -4){proberesult = "Our probe was destroyed when entering sector "+sect2update.toString(16).toUpperCase()+".  No information was gathered.";
							for (cc in clients){
								if (clients[cc].name==results['ownerid'] && clients[cc].gameid==connection.gameid && connection.sectorid){
									clients[cc].sendUTF("Player "+connection.name+" tried to scan us in sector "+sect2update.toString(16).toUpperCase()+" with a probe, but we safely destroyed it and were able to determine that the probe came from sector "+connection.sectorid+".");
								}
								else {
                                                                        clients[cc].sendUTF("Player "+connection.name+" tried to scan us in sector "+sect2update.toString(16).toUpperCase()+" with a probe, but we safely destroyed it in time.");
                                                                }
					...
					if (building2buy==1){
						...
						if (results['metal']>=100*(results2['shipyardlvl']+1)){
                                                ...
                                                if (results['metal']>=100*(results2['orbitalturret']+1)){
                                                        connection.sendUTF('Building a orbital defence turret in sector: '+connection.sectorid.toString(16).toUpperCase());
                                                        client.query('UPDATE map'+connection.gameid+' SET orbitalturret = orbitalturret +1,  groundturret = groundturret +1 WHERE sectorid = "'+connection.sectorid+'"' );
						...
						if (results['metal']>=10000 && results2['warpgate']<1){
                                                        ...
                                                        client.query('UPDATE players'+connection.gameid+' SET metal = metal - '+10000+' WHERE playerid = "'+connection.name+'"' );
							updateResources(connection);
                                                        updateSector2(connection.sectorid, connection);
                                                }
