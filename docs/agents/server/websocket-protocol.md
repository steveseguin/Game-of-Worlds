...
| `namechoice::<json>` | game | Sent only to the player who just swept a shoal: `{sector, chosen, candidates, turn, cost}`. `cost` is the number of hulls lost crossing, and the prompt leads with it. They reply with `//namesector`. Candidates come from `server/lib/sector-names.js` and the server accepts only an index into them, so no player-supplied text ever reaches another player's map. |
