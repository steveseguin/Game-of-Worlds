...
Chill remains defensive even when paired with the Aggressive strategy; strategy no longer overrides the difficulty's military policy. Medium/Aggressive raids retain one ship of each combat hull type at home and require at least two ships in the departing stack. Economy, expansion and research remain active at every difficulty. The waiting-room **Fill with AI & Start** respects the selected profile instead of silently creating aggressive opponents.
