...
| Duplicate existence facilities | Repeated Spaceports or Warp Gates spent resources and slots without adding an effect. | Browser disables duplicates and the server rejects them before capacity or resource writes; a unit regression covers the authoritative boundary. |
