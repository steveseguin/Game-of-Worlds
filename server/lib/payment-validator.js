...
class PaymentValidator {
        ...
        return new Promise((resolve, reject) => {
            this.db.query(
                ...
                (err, results) => {
                    ...
                    this.db.query(
                        'SELECT id, username, email, created FROM users WHERE id = ?',
                        [userId],
                        (fallbackErr, fallbackResults) => {
                            if (fallbackErr) reject(fallbackErr);
                            else resolve(fallbackResults[0] || null);
                        }
                    );
                }
            );
    ...
    async checkSuspiciousActivity(userId) {
        try {
            ...
            console.warn('Payment fraud checks skipped:', error.message || error);
        }
        ...
        return new Promise((resolve, reject) => {
            this.db.query(
                ...
                (err, results) => {
                    ...
                    const raceId = raceMap[productId];
                    if (!raceId) {
                        resolve(false);
                        return;
                    }
                    ...
                    this.db.query(
                        ...
                        (fallbackErr, fallbackResults) => {
                            if (fallbackErr) reject(fallbackErr);
                            else resolve(fallbackResults && fallbackResults.length > 0);
                        }
                    );
                }
            );
